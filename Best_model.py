...
for i in range(1):
	for train, test in kfold.split(X2, Y2):
		...
		# Fit the model		
		history = model[j].fit(X2[train], Y3[train],validation_data=(X2[test], Y3[test]), nb_epoch=200, batch_size=93, class_weight=class_weight, shuffle=True, verbose=1, callbacks=callbacks)#, class_weight=class_weight)
