...
X2 = dataset[10000:230000:,1:31].astype(float)
...
"""
model.add(LocallyConnected2D(32, 3, 3, border_mode='valid', input_shape=(1, 7, 4), activation='relu', W_regularizer=l1l2(l1=0.000005, l2=0.00005)))
model.add(Flatten())  
"""
...
model.add(Dense(120, input_dim=30, init='normal', activation='relu' ,W_regularizer=l1l2(l1=0.000005, l2=0.00005))) #W_regularizer=l1(0.000001), activity_regularizer=activity_l1(0.000001)))
model.add(L)
...
model.compile(optimizer=rms, loss='binary_crossentropy', metrics=['sparse_categorical_accuracy']) # Gradient descent
...
for i in range(1):
	for train, test in kfold.split(X2, Y2):
		...
		# Fit the model		
		model.fit(X2[train], Y3[train],validation_data=(X, Y), nb_epoch=20, batch_size=96, class_weight=class_weight, shuffle=True, verbose=1, callbacks=callbacks)#, class_weight=class_weight)
